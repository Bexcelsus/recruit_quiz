...
QuestionList CreatePoliticsExam()
{
	...
	unordered_map<string, vector<PoliticsData>> data;	//unordered_mapを使っているのは添え字(vectorでいう[n]やat(n))に整数型以外を選べるから
	{	//政治問題データを読み込む
		...
		while (true)
		{
			...
			const vector<string> v = Split(s, ',');	//','でCSV文章を区切る
	...
	//ランダムに選んだ2つのジャンルからジャンル内でランダムに選んだ5問を出題する
