...
QuestionList CreateKanjiExam() {
	static const struct {
		const char* Kanaji;		//漢字
		const char* reading;	//読み
		const char* meanign;	//意味
	} data[] = {
		{"市井","しせい","人が集まって暮らすところ、町"},
		{"捺印","なついん","(署名と共に)印鑑を押すこと"},
		{"相殺","そうさい","足し引きの結果、差が無くなる事"},
		...
		{"吟味","ぎんみ","物事を念入りに調べること"},
		{"承る","うけたまわる","敬意を持って受ける「受ける」「聞く」の謙遜語"},
		{"寸暇","すんか","ほんの少しの空き時間"},
	...
	if (type == 0) {
		...
		for (int i = 0; i < quizCount; i++){
			...
			//問題文を作成
			string s = "「" + string(data[correctIndex].meanign) + "」を意味する熟語の番号を選べ";
			for (int j = 0; j < 3; j++)
			{
				s += "\n  " + to_string(j + 1) + ":" + data[answers[j]].Kanaji;
			}
...
QuestionList CreateIdiomExam() 
{
	...
	constexpr int quizCount = 15; //5+10
	QuestionList questions;
	questions.reserve(quizCount);
	const vector<int> indices = CreateRandomIndices(size(data));
	random_device rd;
